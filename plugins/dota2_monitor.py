...
OPENCLAW_URL = _env("OPENCLAW_URL", "http://127.0.0.1:18789/v1/responses").strip()
OPENCLAW_TOKEN = _env("OPENCLAW_TOKEN", "").strip()
OPENCLAW_MODEL = _env("OPENCLAW_MODEL", "moonshot/kimi-k2.5").strip()
OPENCLAW_AGENT_ID = _env("OPENCLAW_AGENT_ID", "qq_bot").strip()
...
async def _ask_openclaw(prompt: str) -> str:
    if not OPENCLAW_URL or not OPENCLAW_TOKEN:
        raise RuntimeError("OpenClaw is not configured for Dota2 v2.")
    body = {
    ...
    headers = {
        ...
        "x-openclaw-message-channel": "qq-dota2-v2",
    }
    try:
        data = await _post_json(OPENCLAW_URL, body, headers)
        return _extract_openclaw_text(data)
...
async def _build_v2_group_message(match: dict[str, Any], *, target_account_ids: set[str] | None = None) -> str:
    ...
    if target_account_ids is not None:
        payload["tracked_players"] = [
            ...
            if isinstance(player, dict) and str(player.get("account_id")) in target_account_ids
        ]
...
async def _push_recent_matches_with_openclaw(count: int) -> list[str]:
    if count <= 0:
        return []
    await _ensure_name_maps()
...
async def _startup_dota2_monitor() -> None:
    ...
    logger.info(
        ...
        f"accounts={','.join(_watched_account_ids())} interval={DOTA2_POLL_INTERVAL_SECONDS}s"
    )
...
async def _handle_dota_backfill_v2(args: Message = CommandArg()) -> None:
    ...
    async with _poll_lock:
        summaries = await _push_recent_matches_with_openclaw(count)
    if not summaries:
